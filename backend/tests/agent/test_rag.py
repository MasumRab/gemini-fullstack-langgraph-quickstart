...
def test_retrieve_success(rag_classes, mock_config):
    ...
    rag.doc_store[0] = rag_classes.EvidenceChunk(
        content="res1", source_url="url1", subgoal_id="sg1",
        relevance_score=0.9, timestamp=0, chunk_id="c1"
    )
    rag.doc_store[1] = rag_classes.EvidenceChunk(
        content="res2", source_url="url2", subgoal_id="sg1",
        relevance_score=0.8, timestamp=0, chunk_id="c2"
    )
