...
#
# TODO(priority=Medium, complexity=Low, owner=maintenance): [test_mcp:1] Test disabled MCP returns empty list
# - Create MCPSettings with enabled=False
# - Verify get_tools_from_mcp returns []
#
# TODO(priority=Medium, complexity=Medium, owner=maintenance): [test_mcp:2] Test connection error handling
# - Mock SSEConnection to raise ConnectionError
# - Verify graceful fallback (empty list, logged warning)
#
# TODO(priority=Medium, complexity=Medium, owner=maintenance): [test_mcp:3] Test tool whitelist filtering
# - Load multiple tools from mock MCP
# - Set tool_whitelist to subset
# - Verify only whitelisted tools returned
#
# TODO(priority=Low, complexity=Medium, owner=maintenance): [test_mcp:4] Test tool execution with real MCP server
