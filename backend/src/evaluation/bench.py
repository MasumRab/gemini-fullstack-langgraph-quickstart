...
class BenchmarkEvaluator:
    ...
    def evaluate_full_benchmark(
        ...
        for criterion in iterator:
            ...
            try:
                ...
                aggregate_scores["evidence_quality"].append(result["evidence_quality"]["quality_score"])
                aggregate_scores["subgoal_completion"].append(result["subgoal_completion"]["completion_rate"])
                aggregate_scores["hallucination_rate"].append(result["hallucination"]["hallucination_rate"])
                aggregate_scores["context_efficiency"].append(result["context_efficiency"]["efficiency_score"])
        ...
        final_scores = {
            ...
            "evidence_quality": float(np.mean(aggregate_scores["evidence_quality"]) * 100),
            "subgoal_completion": float(np.mean(aggregate_scores["subgoal_completion"]) * 100),
            "hallucination_rate": float(np.mean(aggregate_scores["hallucination_rate"]) * 100),
            "context_efficiency": float(np.mean(aggregate_scores["context_efficiency"])),
        ...
        results = {
            ...
            "context_efficiency": self.metrics.context_efficiency(
                ...
                answer_quality_score=self.metrics.pass_at_1_accuracy(
                    generated_answer, reference["reference_answer"], reference["key_facts"]
                )["score"]
            )
        }
    ...
    def _calculate_overall_score(self, aggregate_scores: Dict) -> float:
        ...
        score += np.mean(aggregate_scores["evidence_quality"]) * weights["evidence_quality"]
        score += np.mean(aggregate_scores["subgoal_completion"]) * weights["subgoal_completion"]
        score += (1 - np.mean(aggregate_scores["hallucination_rate"])) * weights["hallucination_rate"]
        score += min(np.mean(aggregate_scores["context_efficiency"]) / 10, 1.0) * weights["context_efficiency"]
