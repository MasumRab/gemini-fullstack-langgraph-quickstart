...
describe('InputForm', () => {
  ...
  it('allows changing the model and submits with new model', async () => {
    ...
    const changeModelButton = modelSelectRoot!.querySelector(
      'button[data-testid="mock-select-option-gemini-2.5-flash"]'
    )
  ...
  it('allows changing the effort and submits with new effort', async () => {
    ...
    const changeEffortButton = effortSelectRoot!.querySelector(
      'button[data-testid="mock-select-option-high"]'
    )
  ...
  it('has accessible placeholder contrast', () => {
    render(<InputForm {...defaultProps} />)
    const input = screen.getByRole('textbox', { name: /chat input/i })
