...
const CodeBlock = ({ node, inline, className, children, ...props }: any) => {
  ...
  return !inline && match ? (
    <SyntaxHighlighter style={highlighterStyle} language={match[1]} PreTag="div" {...props}>
...
const MarkdownComponents = {
  ...
  h1: ({ node, ...props }: any) => (
    <h1
      className="text-2xl font-bold mt-8 mb-4 text-neutral-100 border-b border-neutral-800 pb-2"
      {...props}
    />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  h2: ({ node, ...props }: any) => (
    <h2 className="text-xl font-semibold mt-6 mb-3 text-neutral-200" {...props} />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  h3: ({ node, ...props }: any) => (
    <h3 className="text-lg font-medium mt-4 mb-2 text-neutral-300" {...props} />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  p: ({ node, ...props }: any) => (
    <p className="text-neutral-400 leading-relaxed mb-4 text-sm md:text-base" {...props} />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ul: ({ node, ...props }: any) => (
    <ul className="list-disc pl-5 mb-4 space-y-2 text-neutral-400" {...props} />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ol: ({ node, ...props }: any) => (
    <ol className="list-decimal pl-5 mb-4 space-y-2 text-neutral-400" {...props} />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ...
  blockquote: ({ node, ...props }: any) => (
    <blockquote
      className="border-l-4 border-blue-500/50 pl-4 italic text-neutral-500 my-4"
      {...props}
    />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ...
  th: ({ node, ...props }: any) => (
    <th
      className="border border-neutral-800 bg-neutral-800/50 px-4 py-2 text-left font-semibold"
      {...props}
    />
  ),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any
  ...
  a: ({ node, ...props }: any) => (
    <a
      className="text-blue-400 hover:text-blue-300 underline decoration-blue-500/30 underline-offset-4 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 rounded-sm"
      target="_blank"
...
export const ArtifactView = React.memo<ArtifactViewProps>(
  ({ content, type, title, isOpen, onClose }) => {
    ...
    return (
      <div
        className={`fixed inset-y-0 right-0 z-50 flex flex-col bg-neutral-900 border-l border-neutral-800 shadow-2xl transition-all duration-300 ease-in-out ${
          isMaximized ? 'left-0' : 'w-full md:w-1/2 lg:w-2/5 xl:w-1/3'
        }`}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-neutral-900 scroll-smooth">
          <div className="max-w-4xl mx-auto p-6 md:p-8">
            <div className="prose prose-invert prose-blue max-w-none">{renderContent()}</div>
          </div>
        </div>
