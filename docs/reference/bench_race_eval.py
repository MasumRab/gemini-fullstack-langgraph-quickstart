...
def _log_error_and_return_none(e):
    logger.exception("Error occurred: " + str(e))
    return None


def _handle_task_error(msg, lock, pbar):
...
def process_single_item(
    ...
    try:
        ...
        _handle_task_error("Formatting error", lock, pbar)
        return {
            "id": task_id,
            "prompt": prompt,
            "error": f"Failed to format criteria: {e!s}",
        }
    ...
    while retry_count < max_retries and not success:
        try:
            ...
            if retry_count < max_retries:
                logger.warning(
                    f"ID {task_id}: Retry {retry_count}/{max_retries} - {e!s}"
                )
                time.sleep(1.5**retry_count)
            else:
                logger.error(
                    f"ID {task_id}: Failed after {max_retries} retries - {e!s}"
                )
    ...
    try:
        ...
        _handle_task_error("Error calculating scores", lock, pbar)
        return {
            "id": task_id,
            "prompt": prompt,
...
def process_language_data(
    ...
    try:
        ...
        target_articles_list = [
            a for a in all_target_articles if a.get("prompt") in task_prompts
        ]
        if not target_articles_list:
            logger.error(
                f"No target articles found for model {target_model} in {language}"
            )
            return None
        ...
        reference_articles_list = [
            a for a in all_reference_articles if a.get("prompt") in task_prompts
        ]
        ...
        reference_articles_map = {
            item["prompt"]: item for item in reference_articles_list
        }
        ...
        for task in all_tasks:
            ...
            if prompt not in target_articles_map:
                logger.warning(
                    f"No target article found for task prompt: {prompt[:50]}..."
                )
            if prompt not in reference_articles_map:
                logger.warning(
                    f"No reference article found for task prompt: {prompt[:50]}..."
                )
    ...
    with tqdm(
        total=len(tasks_to_process), desc=f"Scoring {language} {target_model}"
    ) as pbar:
    ...
    logger.info(
        f"{language} evaluation complete. Successfully scored {len(successful_results)} "
...
def main():
    ...
    parser.add_argument(
        "target_model", type=str, help="Name of target model to evaluate"
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Limit on number of prompts to process (for testing).",
    )
    parser.add_argument(
        "--skip_cleaning", action="store_true", help="Skip article cleaning step."
    )
    parser.add_argument(
        "--only_zh", action="store_true", help="Only process Chinese data."
    )
    parser.add_argument(
        "--only_en", action="store_true", help="Only process English data."
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--cleaned_data_dir",
        type=str,
        default="data/test_data/cleaned_data",
        help="Directory for cleaned data.",
    )
    parser.add_argument(
        "--max_workers", type=int, default=5, help="Maximum number of worker threads."
    )
    parser.add_argument(
        "--query_file",
        type=str,
    ...
    if os.path.exists(output_file) and not force:
        try:
            ...
            logger.info(
                f"Found existing results file with {len(existing_results)} entries"
            )
            ...
            if limit is not None and len(existing_results) >= limit:
                ...
                if successful_results:
                    ...
                    logger.info(
                        f"Instruction Following:  {instruction_following_avg:.4f}"
                    )
            ...
            logger.warning(
                f"Error reading existing results file: {e}. Will create new results."
            )
            existing_results = []
            existing_ids = set()
    ...
    if not only_en:
        ...
        if not skip_cleaning:
            ...
            zh_tasks = [
                task
                for task in all_tasks
                if task.get("language") == "zh" and task.get("id") not in existing_ids
            ]
            if not zh_tasks:
                ...
                existing_zh_count = len(
                    [
                        ...
                        and any(
                            t.get("prompt") == r.get("prompt")
                            and t.get("language") == "zh"
                            for t in all_tasks
                        )
                    ]
                )
                ...
                if remaining_limit > 0:
                    ...
                    logger.info(
                        f"Already reached limit for Chinese tasks ({existing_zh_count}/{limit}). Skipping."
                    )
            else:
    ...
    if not only_zh:
        ...
        if not skip_cleaning:
            ...
            en_tasks = [
                task
                for task in all_tasks
                if task.get("language") == "en" and task.get("id") not in existing_ids
            ]
            if not en_tasks:
                ...
                if remaining_limit > 0:
                    ...
                    logger.info(
                        f"Already reached limit for English tasks ({existing_en_count}/{limit}). Skipping."
                    )
            else:
                # if limit is not specified, process all unprocessed tasks
                en_results = process_language_data(
