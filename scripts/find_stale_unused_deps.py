...
def get_git_blame_date(file_path, line_number):
    try:
        result = subprocess.run(
            ['git', 'blame', '--line-porcelain', '-L', f'{line_number},{line_number}', file_path],
...
def check_js_deps():
    ...
    try:
        ...
        deps = list(package_json.get('dependencies', {}).keys()) + list(package_json.get('devDependencies', {}).keys())
            ...
            if line_num != -1:
                ...
                if date < stale_threshold:
                    ...
                    try:
                        ...
                        subprocess.run(
                            ['grep', '-r', '--exclude-dir=node_modules', '--exclude=package.json', dep, 'frontend/src'],
                            capture_output=True, check=True
                        )
                    except subprocess.CalledProcessError:
                        print(f"Stale/Unused JS Dep: {dep} (Added: {date.strftime('%Y-%m-%d')})")
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('dependencies = ['):
                ...
                dep = line.split('"')[1].split(' ')[0].split('=')[0].split('<')[0].split('>')[0]
                deps.append((dep, i + 1))
        ...
        for dep, line_num in deps:
            ...
            if date < stale_threshold:
                ...
                try:
                    subprocess.run(
                        ['grep', '-r', '--exclude-dir=.venv', f'import {import_name}\\|from {import_name}', 'backend/src'],
                        capture_output=True, check=True
                    )
                except subprocess.CalledProcessError:
                    print(f"Stale/Unused Py Dep: {dep} (Added: {date.strftime('%Y-%m-%d')})")
